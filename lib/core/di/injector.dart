...
import 'package:bibliaexplica/features/bible/data/repositories/verses_repository_impl.dart';
...
void injector() {
  ...
  i.registerFactory<ChaptersDatasource>(
      () => ChaptersDatasource(getIt.get<Future<Database>>()));
  i.registerFactory<VersesDatasource>(
      () => VersesDatasource(getIt.get<Future<Database>>()));

  i.registerFactory<BooksStore>(() => BooksStore(BooksRepositoryImpl()));
  i.registerFactory<ChaptersStore>(
    () => ChaptersStore(
      ChaptersRepositoryImpl(getIt.get<ChaptersDatasource>()),
    ),
  );
  i.registerFactory<VersesStore>(
    () => VersesStore(VersesRepositoryImpl(getIt.get<VersesDatasource>())),
  );
}
